...
def gatescraper():
    ...
    password='User1123!'
    outpath=addpath('tmp/vinterchange/')
    ...
    with Display():
        ...
        #for href in hrefs:
            ...
            for clink in clinks:
                ...
                item=page_soup.find('div',{'class':'quarterpage header-normal header-time-title'})
                try:
                ...
                keys=['RELEASE:','CONTAINER:','SIZE/TYPE:','TRUCK NUMBER:','CHASSIS:','SCALE WT:','GROSS WT:','CARGO WT:','SEALS:','IN TIME:','DRIVER:']
...
def interchange_insert(consets):
    ...
    for obj in consets:
        ...
        idat=Interchange.query.filter((Interchange.CONTAINER==container) & (Interchange.TYPE==typedef)).first()
        if idat is None:
           ...
           # if missing is None:
            print ('Data is being added to database: interchange data for container:',container,' ',typedef)
...
syncline3='rsync -avzhe ssh '+addpath('tmp/vinterchange')+' mnixon@ssh.pythonanywhere.com:/home/mnixon/felrun/tmp/data'           
os.system(syncline3)            
